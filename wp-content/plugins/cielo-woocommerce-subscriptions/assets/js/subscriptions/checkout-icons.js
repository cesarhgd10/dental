(function( $ ) {
	...
	$( function() {
		...
		$( 'body' ).on( 'ajaxComplete', function() {
			$.data( document.body, 'cielo_subscriptions_installments', $( '#cielo-subscriptions-payment-form #cielo-installments' ).html() );
			setInstallmentsFields( $( 'body #cielo-subscriptions-payment-form #cielo-card-brand input' ).first().val() );
		});

		// Set on change the card brand.
		$( 'body' ).on( 'click', '#cielo-subscriptions-payment-form #cielo-card-brand input', function() {
			$( '#cielo-subscriptions-payment-form #cielo-select-name strong' ).html( '<strong>' + $( this ).parent( 'label' ).attr( 'title' ) + '</strong>' );
